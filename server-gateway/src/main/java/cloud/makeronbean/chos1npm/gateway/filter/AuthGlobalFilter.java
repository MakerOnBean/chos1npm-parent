package cloud.makeronbean.chos1npm.gateway.filter;

import cloud.makeronbean.chos1npm.common.result.Result;
import cloud.makeronbean.chos1npm.common.result.ResultCodeEnum;
import cloud.makeronbean.chos1npm.common.util.IpUtil;
import cloud.makeronbean.chos1npm.gateway.constant.RedisConst;
        ...
        if (!StringUtils.isEmpty(authUrls)) {
            ...
            for (String url : split) {
                if (path.contains(url) && StringUtils.isEmpty(userId)){
                    ...
                    // 设置地址
                    response.getHeaders().set(HttpHeaders.LOCATION,"http://localhost:8081/login");
